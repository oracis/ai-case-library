...
PACKS = {
    ...
    "1lookup": {
        ...
        "metrics": {
            "headline": "MRR $244,029；累计收入 $4,985,134（Stripe 直连验证）",
            "mrr": 244029,
            "all_time": 4985134,
        ...
        "why_it_works": [
            ...
            "Stripe 侧直连验证的 MRR 约 $244,029、累计收入约 $498 万—— "
            "证明这类「基础设施型」小工具可以做到年化数百万美元的量级",
            "数字被支付网关直读，不靠创始人自报：这是这个库里可信度最高的一档证据",
        ],
        "signals": [
            "MRR 约 $244,029、累计收入约 $4,985,134（TrustMRR 页面以 Stripe API key 验证）",
    ...
    "stan": {
        ...
        "metrics": {
            "headline": "MRR $3,569,654；累计收入 $76,627,685（Stripe 直连验证）",
            "mrr": 3569654,
            "all_time": 76627685,
        ...
        "why_it_works": [
            "收入已经被支付商 API 直读：MRR 约 $3,569,654、累计收入约 $7,662 万，"
        ...
        "signals": [
            "MRR 约 $3,569,654、累计收入约 $76,627,685（TrustMRR 页面以 Stripe 直连验证）",
    ...
    "magicslides-app": {
        ...
        "metrics": {
            "headline": "MRR $11,332；累计收入 $714,537（Stripe API 验证）",
            "mrr": 11332,
            "all_time": 714537,
            "customers": 825,
        ...
        "why_it_works": [
            ...
            "Stripe 直连验证的 MRR 约 $11,332、累计收入逾 $71 万，"
            "由印度独立开发者单人运行、约 80% 毛利 —— 一个人做到这个量级的样本很稀有",
            "上线时间点（2023-01）刚好踩在生成式 AI 大众认知的前夜，属于「早半年」的窗口红利",
        ],
        "signals": [
            "MRR 约 $11,332、825 个活跃订阅、累计收入约 $714,537（Stripe API 验证）",
    ...
    "autoreels-ai": {
        ...
        "metrics": {
            "headline": "MRR $1,358；累计收入 $35,058（Lemon Squeezy 侧验证）",
            "mrr": 1358,
            "all_time": 35058,
            "customers": 54,
    ...
    "insect-bite-id": {
        ...
        "metrics": {
            "headline": "MRR $2,869；累计收入 $62,352（RevenueCat API 验证）",
            "mrr": 2869,
            "all_time": 62352,
            "customers": 115,
        ...
        "why_it_works": [
            ...
            "—— 一个人可以维护的体量，且 App Store 分发不需要自有获客",
            "App Store 的分发让这类长尾小工具可以长期挂在货架上收订阅，"
            "不需要持续投广告，因为搜索需求本身就在",
        ],
        "signals": [
            "MRR 约 $2,869、115 个活跃订阅、累计收入约 $62,352（RevenueCat API 验证）",
